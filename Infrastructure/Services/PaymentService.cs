    ...
    public class PaymentService : IPaymentService
    {
        ...
        public async Task<CustomerBasket> CreateOrUpdatePaymentIntent(string basketId, CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrEmpty(basket.PaymentIntentId))
            {
                ...
                {
                    Amount = (long)basket.Items.Sum(i => (i.Price * 100)) + (long)shippingPrice * 100,
                    Currency = "usd",
                    PaymentMethodTypes = new List<string> { "card" }
                };
                //intent = await service.CreateAsync(options);
                basket.PaymentIntentId = "IntentId";//intent.Id;
                basket.ClientSecret = "ClientSecret"; //intent.ClientSecret;
            }
            else
            {
